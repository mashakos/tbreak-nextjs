...
export default function Footer() {
  return (
    <footer className="bg-white py-12 sm:py-20 lg:py-24">
      <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-(--breakpoint-xl) lg:px-8">
        {/* Logo & Social Links */}
        <div className="sm:flex sm:items-center sm:justify-between">
          {/* Logo */}
          <div className="flex items-center justify-center">
            <Link href="/" className="block h-10">
              <Image src={logo} alt="Tbreak" className="h-10 w-auto" />
            </Link>
          </div>
        ...
        <div className="mt-10 border-t border-t-gray-300/70 pt-10 md:flex md:items-center md:justify-between">
          ...
          >
            {links.map((item) => (
              <Link
                key={item.label}
                href={item.link}
                className="px-5 py-2 text-base text-gray-500 transition duration-300 ease-in-out hover:text-primaryred"
              >
                {item.label}
              </Link>
            ))}
          ...
          <p className="ml-0 mt-8 flex shrink-0 items-center justify-center text-base text-gray-400 md:ml-6 md:mt-0">
            © {new Date().getFullYear()} Tbreak. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
