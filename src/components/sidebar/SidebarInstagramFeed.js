...
function InstagramImage({ image }) {
  return (
    <div className="group relative z-0 w-full translate-y-0 transform cursor-pointer overflow-hidden rounded-2xl bg-gray-100 pt-full shadow-xs transition duration-300 ease-in-out hover:-translate-y-1">
      <Link
        href={image.href}
      ...
      />
      <div className="absolute inset-0 flex h-full w-full items-center justify-center bg-linear-to-br opacity-0 transition duration-300 ease-in-out group-hover:opacity-90">
        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-100">
          <Instagram className="h-4 w-4 text-secondaryblue" />
        </div>
      </div>
    </div>
  )
